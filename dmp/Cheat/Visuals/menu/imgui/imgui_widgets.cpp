...
void ImGui::Keybind(const char* label, int* key, int* mode, bool label_active)
{
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if ( grab_bb.Max.x > grab_bb.Min.x ) {
        ...
        GetWindowDrawList( )->AddRectFilled(frame_bb.Min + ImVec2(it_anim->second.slow_anim - 2, -1), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 15, grab_bb.Max.y + 3), GetColorU32(it_anim->second.circle), 10.f);
    ...
    ImGui::GlowText(font::inter_bold, 15.f, value_buf, ImVec2(frame_bb.Max.x - CalcTextSize(value_buf).x, frame_bb.Min.y - 22), GetColorU32(c::accent));
...
void render_arrows_for_horizontal_bar(ImVec2 pos, float alpha, float width, float height) {
    ...
    draw->AddRect(pos, pos + ImVec2(width, height), ImColor(0.f, 0.f, 0.f, alpha * ImGui::GetStyle().Alpha), 4);
}
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    GetForegroundd1awList( )->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), col_white, hue_color32, hue_color32, col_white, c::picker::rounding, ImDrawCornerFlags_All);
    GetForegroundd1awList( )->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), 0, 0, col_black, col_black, c::picker::rounding, ImDrawCornerFlags_All);
    ...
    // Render Hue Bar
    for ( int i = 0; i < 6; ++i )
        GetForegroundd1awList( )->AddRectFilledMultiColor(ImVec2(bar0_pos_x + i * ( bars_width / 6 ), bar_pos.y + 6), ImVec2(bar0_pos_x + ( i + 1 ) * ( bars_width / 6 ), bar_pos.y + sv_bar_size - 7), col_hues[i], col_hues[i + 1], col_hues[i + 1], col_hues[i]);
    GetForegroundd1awList( )->AddRectFilledMultiColorRounded(ImVec2(bar0_pos_x, bar_pos.y + 6), ImVec2(bar0_pos_x + bars_width, bar_pos.y + sv_bar_size - 7), GetColorU32(ImGuiCol_PopupBg), 0, 0, 0, 0, 30.f, ImDrawCornerFlags_All);
    ...
    GetForegroundd1awList( )->AddRectFilled(ImVec2(it_anim->second.hue_bar + bar_pos.x - 3, bar_pos.y + 3), ImVec2(it_anim->second.hue_bar + bar_pos.x + 5, bar_pos.y + 19), ImColor(255, 255, 255, 255), 30.f);
    ...
    GetForegroundd1awList( )->AddCircleFilled(it_anim->second.circle_move + bar_pos, it_anim->second.circle, user_col32_striped_of_alpha, 32);
    GetForegroundd1awList( )->AddCircle(it_anim->second.circle_move + bar_pos + ImVec2(0, 1), it_anim->second.circle, IM_COL32(50, 50, 50, 150 * g.Style.Alpha), 32);
    GetForegroundd1awList( )->AddCircle(it_anim->second.circle_move + bar_pos, it_anim->second.circle, IM_COL32(230, 230, 230, 255 * g.Style.Alpha), 32);
    ...
    if ( alpha_bar )
    {
        ...
        GetForegroundd1awList( )->AddRectFilledMultiColor(bar1_bb.Min + ImVec2(0, 8), bar1_bb.Max - ImVec2(0, 5), user_col32_striped_of_alpha & ~IM_COL32_A_MASK, user_col32_striped_of_alpha, user_col32_striped_of_alpha, user_col32_striped_of_alpha & ~IM_COL32_A_MASK);
        GetForegroundd1awList( )->AddRectFilledMultiColorRounded(bar1_bb.Min + ImVec2(0, 8), bar1_bb.Max - ImVec2(0, 5), GetColorU32(ImGuiCol_PopupBg), 0, 0, 0, 0, c::picker::rounding, ImDrawCornerFlags_All);
        ...
        GetForegroundd1awList( )->AddCircleFilled(ImVec2(it_anim->second.alpha_bar + bar_pos.x, bar1_bb.Min.y + 13.0f), 6, IM_COL32(255, 255, 255, 255), 100.f);
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    //if (g.IO.KeyCtrl) { GetForegroundd1awList()->AddRect(bb.Min, bb.Max, IM_COL32(0, 255, 0, 255)); }
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (g.HoveredWindow == window && child_menu_window != NULL)
        {
            ...
            //GetForegroundd1awList()->AddTriangleFilled(ta, tb, tc, moving_toward_child_menu ? IM_COL32(0,128,0,128) : IM_COL32(128,0,0,128)); // [DEBUG]
        }
...
void ImGui::TabBarQueueReorderFromMousePos(ImGuiTabBar* tab_bar, ImGuiTabItem* src_tab, ImVec2 mouse_pos)
{
    ...
    for (int i = src_idx; i >= 0 && i < tab_bar->Tabs.Size; i += dir)
    {
        ...
        //GetForegroundd1awList()->AddRect(ImVec2(x1, tab_bar->BarRect.Min.y), ImVec2(x2, tab_bar->BarRect.Max.y), IM_COL32(255, 0, 0, 255));
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    if (hovered && g.HoveredIdNotActiveTimer > TOOLTIP_DELAY && bb.GetWidth() < tab->ContentWidth)
    {
        ...
        display_draw_list = GetForegroundd1awList(window);
