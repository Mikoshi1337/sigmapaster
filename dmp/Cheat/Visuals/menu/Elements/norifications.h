...
enum ImGuiToastType_
{
	...
	ImGuiToastType_Reload,
	ImGuiToastType_Warning,
	ImGuiToastType_Hiden,
...
public:
	...
	NOTIFY_INLINE auto get_default_title( ) -> char*
	{
		if ( !strlen(this->title) )
		{
			...
			case ImGuiToastType_Success:
				return "Success";
			case ImGuiToastType_Reload:
				return "Reload";
			case ImGuiToastType_Warning:
				return "Warning";
			case ImGuiToastType_Hiden:
				return "Hiden";
	...
	NOTIFY_INLINE auto get_color( ) -> const ImVec4&
	{
		...
		case ImGuiToastType_Success:
			return { 0, 255, 0, 255 };
		case ImGuiToastType_Reload:
			return { 255, 183, 0, 255 };
		case ImGuiToastType_Warning:
			return { 255, 255, 0, 255 };
		case ImGuiToastType_Hiden:
			return { color_ui[0], color_ui[1], color_ui[2], 255 };
		case ImGuiToastType_Error:
			return { 255, 0, 0, 255 };
		case ImGuiToastType_Info:
	...
	NOTIFY_INLINE VOID RenderNotifications( )
	{
		...
		for ( auto i = 0; i < notifications.size( ); i++ )
		{
			...
			Begin(window_name, NULL, NOTIFY_TOAST_FLAGS);
			{
				...
				GetForegroundd1awList( )->AddRectFilled(pos + ImVec2(20, region.y - 16), pos + ImVec2(region.x, region.y - 10), GetColorU32(c::child::outline), 30.f);

				//GetForegroundd1awList( )->AddShadowRect(pos + ImVec2(20, region.y - 16), pos + ImVec2(opacity * region.x, region.y - 10), GetColorU32(text_color), 10.f, ImVec2(0, 0), 30.f);
				GetForegroundd1awList( )->AddRectFilled(pos + ImVec2(20, region.y - 16), pos + ImVec2(opacity * region.x, region.y - 10), GetColorU32(text_color), 30.f);
				...
				TextColored(text_color, "relax.project");
				PopFont( );
