...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImRect bb(pos + ImVec2(5, 5), pos + ImVec2(5, 5) + size);
    ...
    TabItemMap->second.CheckboxFrame = ImLerp(TabItemMap->second.CheckboxFrame.Value, held || hovered ? ImColor(accent_color[0], accent_color[1], accent_color[2]) : ImColor(46, 46, 46), g.IO.DeltaTime * 8.f * 0.8);

    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(19, 19, 19), style.FrameRounding);
    window->DrawList->AddRect(bb.Min, bb.Max, TabItemMap->second.CheckboxFrame, style.FrameRounding);
    if (g.LogEnabled)
...
#include <map>
bool ImGui::Checkbox(const char* label, bool* v, bool settings_menu)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos + ImVec2(5, 5), pos + ImVec2(5 + square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), 5 + label_size.y + style.FramePadding.y * 2.0f));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    const ImRect check_bb(pos + ImVec2(5, 5), pos + ImVec2(5 + square_sz, 5 + square_sz));
    struct TabAnimation {
        ImColor CheckboxFrame;
    };
    ...
    TabItemMap->second.CheckboxFrame = ImLerp(TabItemMap->second.CheckboxFrame.Value, *v ? ImColor(accent_color[0], accent_color[1], accent_color[2]) : ImColor(0, 0, 0, 0), g.IO.DeltaTime * 8.f * 0.8);

    RenderNavHighlight(total_bb, id);
   
    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(19, 19, 19), style.FrameRounding);

    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, TabItemMap->second.CheckboxFrame, style.FrameRounding);

    window->DrawList->AddRect(check_bb.Min, check_bb.Max, ImColor(46, 46, 46), style.FrameRounding);
    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y - 1);
    RenderText(label_pos, label);
    if (settings_menu)
        if (ImGui::IsItemHovered())
            ImGui::SetTooltip("Click Right Mouse Button For Open settings menu.");
    //RenderText(label_pos + ImVec2(text_size,0), "  Right Mouse Button");
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos + ImVec2(5, 5), window->DC.CursorPos + ImVec2(5 + w, 5 + label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(bb.Min + ImVec2(5, 5), bb.Max + ImVec2(5 + label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 5 + 0.0f));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    TabItemMap->second.CheckboxFrame = ImLerp(TabItemMap->second.CheckboxFrame.Value, popup_open || hovered ? ImColor(accent_color[0], accent_color[1], accent_color[2]) : ImColor(46, 46, 46), g.IO.DeltaTime * 8.f * 0.8);

    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(19, 19, 19), style.FrameRounding);
    window->DrawList->AddRect(bb.Min, bb.Max, popup_open || hovered ? ImColor(accent_color[0], accent_color[1], accent_color[2]) : ImColor(TabItemMap->second.CheckboxFrame), style.FrameRounding);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(5, 15), window->DC.CursorPos + ImVec2(5 + w, 15 + label_size.y + style.FramePadding.y - 6));
    const ImRect total_bb(frame_bb.Min + ImVec2(5, 3), frame_bb.Max + ImVec2(5 + label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 3));
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Max.x, frame_bb.Max.y), ImColor(accent_color[0], accent_color[1], accent_color[2]), style.FrameRounding);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(46, 46, 46), style.FrameRounding);
    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    if (hovered)
        window->DrawList->AddText(ImVec2(frame_bb.Max.x - 2 - CalcTextSize(value_buf).x, window->DC.CursorPos.y - 14), ImColor(255, 255, 255), value_buf, value_buf_end);
    window->DrawList->AddRectFilled(ImVec2(grab_bb.Max.x, frame_bb.Min.y), ImVec2(grab_bb.Max.x + 2, frame_bb.Min.y + 9), ImColor(255, 255, 255), style.GrabRounding);
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x + style.ItemInnerSpacing.x - 4, frame_bb.Min.y - 15), label);
    ImGui::Spacing();
    ImGui::Spacing();
    ImGui::Spacing();
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(5, 5), window->DC.CursorPos + frame_size + ImVec2(5, 5));
    ...
    if (is_multiline)
    {
        ...
        /// const ImColor bg_col = ImColor((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImColor(accent_color[0], accent_color[1], accent_color[2]) : ImColor(42, 42, 43));
        PushStyleColor(ImGuiCol_ChildBg, ImVec4(ImColor(42, 42, 43)));
        PushStyleVar(ImGuiStyleVar_ChildRounding, 4);
    ...
    TabItemMap->second.CheckboxFrame = ImLerp(TabItemMap->second.CheckboxFrame.Value, hovered ? ImColor(accent_color[0], accent_color[1], accent_color[2]) : ImColor(46, 46, 46), g.IO.DeltaTime * 8.f * 0.8);

    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(19, 19, 19), style.FrameRounding);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, TabItemMap->second.CheckboxFrame, style.FrameRounding);
    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos - ImVec2(5, 1) : frame_bb.Min + style.FramePadding - ImVec2(5, 1);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    window->DC.CursorPos.x = pos.x + inputs_offset_x + 5;
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        window->DC.CursorPos = ImVec2(pos.x + button_offset_x + 5, pos.y + 5);

        const ImVec4 col_v4(col[0], col[1], col[2], alpha ? col[3] : 1.0f);
        if (ColorButton("##ColorButton", col_v4, flags))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))//ImGuiColorEditFlags_Pallet
            {
        ...
        if (BeginPopup("picker"))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                ImGuiColorEditFlags picker_flags = (flags_untouched & picker_flags_to_forward) | ImGuiColorEditFlags_DisplayMask_ | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_NoInputs;
                SetNextItemWidth(square_sz * 12.0f); // Use 256 + bar sizes?
                value_changed |= ColorPicker4("##picker", col, picker_flags, &g.ColorPickerRef.x);
            }
            if ((flags & ImGuiColorEditFlags_Pallet))
            {//ImGuiColorEditFlags_Pallet
                ...
                for (int n = 0; n < IM_ARRAYSIZE(saved_palette); n++)
                {
                    ...
                    ImGui::ColorButton("##palette", saved_palette[n], palette_button_flags, ImVec2(20, 20));
                    //  col = ImVec4(saved_palette[n], saved_palette[n], saved_palette[n]); // Preserve alpha!
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        window->DC.CursorPos.x = pos.x + ((flags & ImGuiColorEditFlags_NoInputs) ? w_button : w_full + style.ItemInnerSpacing.x)+2;
        TextEx(label, label_display_end);
    }
...
ImVec2 GetCursorPosDD() {
    ImGuiWindow* window = ImGui::GetCurrentWindowRead();
    return window->DC.CursorPos - window->Pos + window->Scroll;
}


void ColorTooltip_(const char* text, ImColor color, float alpha)
{
    ImGui::PushStyleVar(ImGuiStyleVar_Alpha, alpha);
    ImGui::PushStyleVar(ImGuiStyleVar_WindowBorderSize, 0);
    ImGui::PushStyleVar(ImGuiStyleVar_PopupBorderSize, 1);
    ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(35 / 255.f, 35 / 255.f, 35 / 255.f, alpha));

    ImGui::SetNextWindowPos(ImGui::GetWindowPos() + GetCursorPosDD() + ImGui::GetStyle().WindowPadding);
    ImGui::BeginTooltipEx(ImGuiTooltipFlags_None, ImGuiTooltipFlags_::ImGuiTooltipFlags_None);
    ImGui::ColorButton("", ImVec4(color.Value.x, color.Value.y, color.Value.z, alpha)); ImGui::SameLine();
    ImGui::Text(text);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_InputRGB)
    {
        ...
        if (memcmp(g.ColorEditLastColor2, col, sizeof(float) * 3) == 0)
        {
            if (S == 0)
                H = g.ColorEditLastHue;
            if (V == 0)
    ...
    if (value_changed_h || value_changed_sv)
    {
        if (flags & ImGuiColorEditFlags_InputRGB)
        {
            ...
            memcpy(g.ColorEditLastColor2, col, sizeof(float) * 3);
        }
        else if (flags & ImGuiColorEditFlags_InputHSV)
        {
            col[0] = H;
    ...
    if (value_changed)
    {
        if (flags & ImGuiColorEditFlags_InputRGB)
        {
            ...
            B = col[2];
            ColorConvertRGBtoHSV(R, G, B, H, S, V);
            if (memcmp(g.ColorEditLastColor2, col, sizeof(float) * 3) == 0) // Fix local Hue as display below will use it immediately.
            {
                if (S == 0)
                    H = g.ColorEditLastHue;
                if (V == 0)
    ...
    bool show_color[2] = { false, false };
    static float alpha_here_[2] = { 0, 0 };

    //auto si = GetWindowSize();
    if (Button("Copy"))
        show_color[0] = true;
    SameLine(0, 6);
    if (Button("Paste"))
    ...
    if (show_color[0])
    {
        ...
        if (alpha_here_[0] <= 1.5f)
            alpha_here_[0] += 1.5f;
    }
    else
    {
        if (alpha_here_[0] >= 0.f)
            alpha_here_[0] -= 0.025f;
    }
    if (show_color[1])
    {
        memcpy(col, color_buffer, components * sizeof(float));
        value_changed = true;
        if (alpha_here_[1] <= 1.5f)
            alpha_here_[1] += 1.5f;
    }
    else
    {
        if (alpha_here_[1] >= 0.f)
            alpha_here_[1] -= 0.025f;
    }
    ImU32 end_colo = ImColor(color_buffer[0], color_buffer[1], color_buffer[2]);
    if (alpha_here_[0] > 0)
    {
    
        ColorTooltip_("Copied", end_colo, alpha_here_[0]);
    }
    if (alpha_here_[1] > 0)
    {
    
        ColorTooltip_("Pasted", end_colo, alpha_here_[1]);
    }
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    {
        // Framed type
        //window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(25, 25, 25), 4);
        //window->DrawList->AddRect(bb.Min, bb.Max, ImColor(45, 45, 45), 4);
        //if (hovered || pressed)
        //    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(accent_color[0], accent_color[1], accent_color[2]), 4);

        const ImColor bg_col = ImColor((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImColor(accent_color[0], accent_color[1], accent_color[2]) : ImColor(42, 42, 43));
        RenderFrame(frame_bb.Min, frame_bb.Max, bg_col, true, 4);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    TabItemMap->second.CheckboxFrame = ImLerp(TabItemMap->second.CheckboxFrame.Value, hovered || selected ? ImColor(accent_color[0], accent_color[1], accent_color[2]) : ImColor(143, 143, 143), g.IO.DeltaTime * 8.f * 0.8);
    TabItemMap->second.position = ImLerp(TabItemMap->second.position, hovered || selected ? 5.f : 0.f, g.IO.DeltaTime * 8.f * 0.8);

    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(TabItemMap->second.CheckboxFrame));
    RenderTextClipped(text_min + ImVec2(TabItemMap->second.position, 0), text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
    ImGui::PopStyleColor();
    // Automatically close popups
